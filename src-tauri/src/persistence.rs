...
pub fn load_from_directory(directory: &Path) -> Result<Option<Value>, String> {
    let candidates = [
...
mod tests {
    ...
    fn load(directory: &Path) -> Result<Option<serde_json::Value>, String> {
    ...
    fn round_trips_workspace_json() {
        ...
        let workspace = r#"{"version":2,"selectedThreadId":"thread-1","threads":[{"id":"thread-1","messages":[{"text":"hello"}]}]}"#;

        save_to_directory(directory.path(), workspace).expect("workspace should save");

        assert_eq!(
            load(directory.path()).expect("workspace should load"),
            Some(serde_json::from_str(workspace).expect("workspace should be valid JSON"))
        );
    ...
    fn a_second_save_replaces_the_previous_snapshot() {
        let directory = tempfile::tempdir().expect("test directory should be created");
        save_to_directory(directory.path(), r#"{"version":2,"threads":[]}"#)
            .expect("first workspace should save");
        let latest = r#"{"version":2,"threads":[{"id":"latest"}]}"#;

        save_to_directory(directory.path(), latest).expect("latest workspace should save");

        assert_eq!(
            load(directory.path()).expect("workspace should load"),
            Some(serde_json::from_str(latest).expect("workspace should be valid JSON"))
        );
    ...
    fn loads_the_backup_when_the_primary_file_is_oversized() {
        let directory = tempfile::tempdir().expect("test directory should be created");
        let workspace = r#"{"version":2,"threads":[{"id":"recoverable"}]}"#;
