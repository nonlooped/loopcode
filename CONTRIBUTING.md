...
- **Nightly** is a GitHub pre-release. Master CI starts one after `CI / gate` when the push changes `src/`, `src-tauri/`, or `rust-toolchain.toml`, and is not a stable version bump. Docs-only and workflow-only merges skip it. A newer nightly cancels an in-flight nightly and never a stable. The app version is `{current-stable}-nightly.{date}.{run}`, for example `0.8.0-nightly.20260825.12`. Nightlies do not become GitHub Latest and do not bump `package.json` on `master`. CI dispatches the release workflow from a `nightly-src-<sha>` tag so the YAML matches that commit.
