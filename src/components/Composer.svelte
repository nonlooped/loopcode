...
<section class:picker-open={modelPickerOpen || reasoningPickerOpen} class="composer-wrap">
  <div class="composer" class:working={status === 'running'}>
    {#if props.images.length > 0 || props.attachmentError}
      <div class="attachment-strip" aria-label="Attached images">
        {#each props.images as image (image.id)}
          <div class="image-attachment" title={image.name}>
    ...
    <button class="attach-button" type="button" aria-label="Attach images" title="Attach images (you can also paste them)" disabled={!canEdit()} onclick={() => imageInput?.click()}>
