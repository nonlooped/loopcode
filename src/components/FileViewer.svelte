...
<section class="flex min-h-0 min-w-0 flex-1 flex-col" aria-label={`File viewer: ${name}`}>
  <header class="grid min-h-[39px] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 border-b border-line px-2.5 py-1.5">
    <div class="flex gap-0.5">
      <button type="button" class="grid size-[25px] place-items-center rounded-md text-muted hover:bg-panel-hover hover:text-ink-soft disabled:opacity-35" aria-label="Back" title="Back" onclick={back}>
        <IconArrowLeft size={15} stroke={1.55} />
      </button>
      <button
        type="button"
        class="grid size-[25px] place-items-center rounded-md text-muted hover:bg-panel-hover hover:text-ink-soft disabled:opacity-35"
        aria-label="Forward"
        title="Forward"
    ...
    <div class="flex min-w-0 items-baseline justify-center gap-2 overflow-hidden whitespace-nowrap" title={path}>
      <strong class="truncate text-xs font-semibold text-ink-soft">{name}</strong>
      {#if !mediaType}<small class="truncate text-[11px] text-faint">{language}</small>{/if}
    </div>
    <button type="button" class="grid size-[25px] place-items-center rounded-md text-muted hover:bg-panel-hover hover:text-ink-soft" aria-label="Close file viewer" title="Back to conversation" onclick={close}>
  ...
  <div class="min-h-0 min-w-0 flex-1 overflow-auto">
    ...
    {:else if imageUrl}
      <div class="grid min-h-full min-w-full place-items-center p-6"><img class="block max-h-full max-w-full object-contain" src={imageUrl} alt={name} /></div>
    {:else if documentType === 'markdown'}
      <article class="message-body mx-auto w-[min(100%,816px)] px-[clamp(15px,5.8vw,48px)] pt-8 pb-14">
        <MarkdownMessage id={`file:${path}`} source={content} />
      </article>
    {:else if documentType === 'html'}
      <iframe class="block size-full border-0 bg-[var(--shell-tint)]" title={`Preview of ${name}`} sandbox="" srcdoc={content}></iframe>
    {:else}
      <div class="[&_.line]:block [&_.line]:min-h-[1.6em] [&_.line]:pr-5 [&_.line]:pl-[52px] [&_.line]:indent-[-52px] [&_.line::before]:inline-block [&_.line::before]:w-[52px] [&_.line::before]:pr-3.5 [&_.line::before]:text-right [&_.line::before]:text-faint [&_.line::before]:content-[counter(file-line)] [&_code]:flex [&_code]:flex-col [&_code]:counter-reset-[file-line] [&_pre]:m-0 [&_pre]:min-h-full [&_pre]:w-full [&_pre]:bg-transparent! [&_pre]:pt-3.5 [&_pre]:pb-8 [&_pre]:font-mono [&_pre]:text-xs [&_pre]:leading-[1.6] [&_pre]:whitespace-pre-wrap [&_pre]:wrap-anywhere [&_pre]:[tab-size:2]">{@html highlighted}</div>
    {/if}
  </div>
</section>
